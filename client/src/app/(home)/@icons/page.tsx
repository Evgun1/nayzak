				...
				CONTENT.map((value, index) => (
					...
					>
						<div className={classes["icon-box__content"]}>
							<DisplayIcon
								className={classes["icon-box__icon"]}
								iconName={value.icon}
							/>
							<div className={classes["icon-box__info"]}>
								<div
									className={`${classes["icon-box__info-item"]} ${ButtonClassList.BUTTON_MEDIUM}`}
								>
									{value.title}
								</div>
								<span
									className={`${classes["icon-box__info-item"]} ${TextClassList.REGULAR_14}`}
								>
									{value.span}
								</span>
							</div>
